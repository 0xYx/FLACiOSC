...
#ifdef _MSC_VER
                                flac__utils_printf(stderr, 1, "%s: WARNING: unexpected EOF; expected % I64u  samples, got % I64u  samples\n", encoder_session.inbasefilename, encoder_session.total_samples_to_encode, encoder_session.samples_written);
#else
                                flac__utils_printf(stderr, 1, "%s: WARNING: unexpected EOF; expected % llu  samples, got % llu  samples\n", encoder_session.inbasefilename, encoder_session.total_samples_to_encode, encoder_session.samples_written);
#endif
				...
				while(encoder_session.fmt.iff.data_bytes > 0) {
					...
					if(bytes_read == 0) {
						if(ferror(infile)) {
							...
							if(options.ignore_chunk_sizes) {
#ifdef _MSC_VER
								flac__utils_printf(stderr, 1, "%s: INFO: hit EOF with --ignore-chunk-sizes, got % I64u samples\n", encoder_session.inbasefilename, encoder_session.samples_written);
#else
                                flac__utils_printf(stderr, 1, "%s: INFO: hit EOF with --ignore-chunk-sizes, got % llu samples\n", encoder_session.inbasefilename, encoder_session.samples_written);
#endif
							}
							else {
#ifdef _MSC_VER
...
#ifdef _MSC_VER
						flac__utils_printf(stderr, 1, "%s: WARNING: unexpected EOF; read % I64u bytes; expected % I64u samples, got % I64u samples\n", encoder_session.inbasefilename, bytes_read, encoder_session.total_samples_to_encode, encoder_session.samples_written);
#else
                        flac__utils_printf(stderr, 1, "%s: WARNING: unexpected EOF; read % llu bytes; expected % llu samples, got % llu samples\n", encoder_session.inbasefilename, bytes_read, encoder_session.total_samples_to_encode, encoder_session.samples_written);
#endif
						if(encoder_session.treat_warnings_as_errors)
							return EncoderSession_finish_error(&encoder_session);
					}
...
#ifdef _MSC_VER
			stats_print_info(2, "%swrote % Iu64u bytes, ratio=%s",
				encoder_session->verify? "Verify OK, " : "",
				encoder_session->bytes_written,
				ratiostr
			);
#else
            stats_print_info(2, "%swrote % llu bytes, ratio=%s",
                             encoder_session->verify? "Verify OK, " : "",
                             encoder_session->bytes_written,
                             ratiostr
                             );
...
#ifdef _MSC_VER
	flac__utils_printf(stderr, 1, "       Absolute sample=% I64u, frame=%u, channel=%u, sample=%u, expected %d, got %d\n", absolute_sample, frame_number, channel, sample, expected, got);
#else
    	flac__utils_printf(stderr, 1, "       Absolute sample=% llu, frame=%u, channel=%u, sample=%u, expected %d, got %d\n", absolute_sample, frame_number, channel, sample, expected, got);
#endif
